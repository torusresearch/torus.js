...
describe("torus utils sapphire", function () {
  ...
  it("should be able to login a v1 user", async function () {
    ...
    expect(retrieveSharesResponse).eql({
      ...
      finalKeyData: {
        evmAddress: "0xbeFfcC367D741C53A63F50eA805c1e93d3C64fEc",
        X: "19499419649390845580457764508395315278888806091504683223990787274846316359249",
        Y: "115319130816082515337172443545355478994523337895659677857630000207487199830447",
        privKey: "dca7f29d234dc71561efe1a874d872bf34f6528bc042fe35e57197eac1f14eb9",
      },
      metadata: {
  ...
  it("should fetch user type and public address of legacy v2 user", async function () {
    ...
    expect(result.finalPubKeyData.evmAddress).to.equal("0xE91200d82029603d73d6E307DbCbd9A7D0129d8D");
    ...
    expect(data.finalPubKeyData.evmAddress).to.equal("0x1016DA7c47A04C76036637Ea02AcF1d29c64a456");
    expect(data).eql({
      oAuthPubKeyData: {
  ...
  it("should fetch public address", async function () {
    ...
    expect(result.finalPubKeyData.evmAddress).to.equal("0x4924F91F5d6701dDd41042D94832bB17B76F316F");
    expect(result).eql({
      oAuthPubKeyData: {
  ...
  it("should keep public address same", async function () {
    ...
    expect(result1.finalPubKeyData).eql(result2.finalPubKeyData);
    expect(result1.oAuthPubKeyData).eql(result2.oAuthPubKeyData);
  ...
  it("should be able to key assign", async function () {
    ...
    const torusNodeEndpoints = nodeDetails.torusNodeSSSEndpoints;
    const result = await torus.getPublicAddress(torusNodeEndpoints, nodeDetails.torusNodePub, verifierDetails);
    expect(result.finalPubKeyData.evmAddress).to.not.equal("");
    expect(result.finalPubKeyData.evmAddress).to.not.equal(null);
  });
  ...
  it("should be able to login", async function () {
    ...
    expect(result).eql({
      finalKeyData: {
        evmAddress: "0x4924F91F5d6701dDd41042D94832bB17B76F316F",
        X: "110327163864614539882969823890367397694842614496454360857202707485912280304639",
        Y: "110240385663407393798240836558291577308287313391499935749499821306565530874317",
  ...
  it("should be able to import a key for a existing user", async function () {
    ...
    const result2 = await torus.getPublicAddress(torusNodeEndpoints, nodeDetails.torusNodePub, verifierDetails);
    expect(result1.finalKeyData.evmAddress).to.be.equal(result2.finalPubKeyData.evmAddress);
  });
  ...
  it("should fetch pub address of tss verifier id", async function () {
    ...
    const torusNodeEndpoints = nodeDetails.torusNodeSSSEndpoints;
    const result = await torus.getPublicAddress(torusNodeEndpoints, nodeDetails.torusNodePub, verifierDetails);
    expect(result.finalPubKeyData.evmAddress).to.be.equal("0xBd6Bc8aDC5f2A0526078Fd2016C4335f64eD3a30");
    expect(result).eql({
      oAuthPubKeyData: {
  ...
  it("should assign key to tss verifier id", async function () {
    ...
    const result = await torus.getPublicAddress(torusNodeEndpoints, nodeDetails.torusNodePub, verifierDetails);
    expect(result.finalPubKeyData.evmAddress).to.not.equal(null);
    expect(result.oAuthPubKeyData.evmAddress).to.not.equal(null);
    expect(result.metadata.typeOfUser).to.equal("v2");
  ...
  it("should fetch public address when verifierID hash enabled", async function () {
    ...
    const torusNodeEndpoints = nodeDetails.torusNodeSSSEndpoints;
    const result = await torus.getPublicAddress(torusNodeEndpoints, nodeDetails.torusNodePub, verifierDetails);
    expect(result.finalPubKeyData.evmAddress).to.equal("0xF79b5ffA48463eba839ee9C97D61c6063a96DA03");
    expect(result).eql({
      oAuthPubKeyData: {
  ...
  it("should be able to login when verifierID hash enabled", async function () {
    ...
    expect(result).eql({
      finalKeyData: {
        evmAddress: "0xF79b5ffA48463eba839ee9C97D61c6063a96DA03",
        X: "15288602791273750074006916629209422708636155719321341944582659737320049233290",
        Y: "39512845000186250107791179215691362545116071539007469639341362275861040486942",
