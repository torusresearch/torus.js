...
import log from "../src/loglevel";
import TorusUtils from "../src/torus";
import { keyAssign } from "../src/utils";
...
describe("New Network tests", function () {
  let torus;
  ...
  // it("runs key assign", async function () {
  ...
  it("should be able to do full flow", async function () {
    ...
    // const { torusNodeEndpoints, torusNodePub } = await TORUS_NODE_MANAGER.getNodeDetails(verifierDetails);
    //  torusNodePub
    const res = await keyAssign({ endpoints, verifier: verifierContract, verifierId: verifierDetails.verifierId });
    ...
    for (let i = 0; i < res[i].keys.length; i++) {
      ...
      const pubkey = torus.ec.keyFromPublic({ x: pkStr.x, y: pkStr.y }).getPublic();
    ...
    // log.info("keyassign res", finalPubKeyFromNodes.getX().toString("hex"), finalPubKeyFromNodes.getY().toString("hex"));

    // eslint-disable-next-line promise/param-names
    await new Promise((r) => setTimeout(r, 2000));
    const pkNTokens = await torus.retrieveShares(endpoints, verifierContract, verifierDetails, mockToken, true);
    // log.info("pkNTokens", pkNTokens.pubkey_x, pkNTokens.pubkey_y);
    expect(finalPubKeyFromNodes.getX().toString("hex")).to.equal(pkNTokens.pubkey_x);
  });
});
