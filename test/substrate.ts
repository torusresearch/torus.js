...
import TorusUtils from "../src/torus";
 import { generateIdToken, randomEmail } from "./helpers";
...
describe("torus YOIUIOUWROIEU", function () {
  ...
  it("should be able to do full flow", async function () {
    const verifierContract = "5G7BfNLCMY3fRnB1sMWHZMk63fn34LjJAxXqmfC1cBKd8jja"; // replacement for verifierName/ identifier
    //const mockToken =
    ...
    const endpoints = ["http://[::1]:8800", "http://[::1]:8801", "http://[::1]:8802"];
    ...
    // const { torusNodeEndpoints, torusNodePub } = await TORUS_NODE_MANAGER.getNodeDetails(verifierDetails);
    //  torusNodePub
    let token = generateIdToken(email, "ES256")
    await torus.retrieveShares(endpoints, verifierContract, verifierDetails, token, true);
